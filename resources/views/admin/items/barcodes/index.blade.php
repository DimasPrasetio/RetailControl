                    ...
                    <div class="flex flex-shrink-0 items-center gap-2">
                        ...
                        @else
                            <form method="POST" action="{{ route('admin.items.barcodes.set-primary', [$item, $barcode]) }}">
                                ...
                                @method('PATCH')
                                <button type="submit"
                                    class="inline-flex items-center gap-1 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-600 shadow-sm transition hover:bg-indigo-100"
                        ...
                        <form method="POST" action="{{ route('admin.items.barcodes.destroy', [$item, $barcode]) }}"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-500 transition hover:border-red-300 hover:bg-red-100 hover:text-red-700"
                                title="Hapus barcode">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
