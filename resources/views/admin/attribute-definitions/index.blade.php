    ...
    <form method="GET" action="{{ route('admin.attribute-definitions.index') }}" class="flex flex-wrap items-end gap-3">
        @if(auth()->user()->isPlatformAdmin())
            <div class="min-w-[220px]">
                <label class="mb-1 block text-xs font-medium text-gray-600">Perusahaan</label>
                    ...
                    <tr class="hover:bg-blue-50/40">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-1.5">
                                ...
                                @endcan
                                @can('delete', $attributeDefinition)
                                    <form id="delete-attrdef-{{ $attributeDefinition->id }}" method="POST" action="{{ route('admin.attribute-definitions.destroy', $attributeDefinition) }}" style="display:none;">
                                        @csrf @method('DELETE')
                                    </form>
                                    <button type="button"
                                        onclick="window.dispatchEvent(new CustomEvent('open-delete-modal', { detail: { formId: 'delete-attrdef-{{ $attributeDefinition->id }}', itemName: '{{ addslashes($attributeDefinition->label) }}' } }))"
