    ...
    <div class="mb-5 rounded-2xl bg-white p-4 shadow-xl shadow-indigo-500/10 border border-slate-300">
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-wrap items-end gap-3">
            ...
            <div class="min-w-[160px]">
                ...
                <select name="role_id"
                    ...
                    @foreach($roles as $role)
                        <option value="{{ $role->id }}" {{ request('role_id') == $role->id ? 'selected' : '' }}>{{ $role->name->label() }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['q', 'role_id', 'active']))
                                ...
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    @can('delete', $user)
                                        <form method="POST" action="{{ route('admin.users.destroy', $user) }}"
                                            onsubmit="confirmAction(event, this, 'Konfirmasi Hapus', 'Hapus permanen user {{ addslashes($user->name) }}? Data akan di-soft-delete.')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
