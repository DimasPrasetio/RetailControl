...
- role global tenant aktif
- audit trail aktif
- soft delete user aktif

Arsitektur saat ini masih lean:
- Laravel monolith
- authorization memakai policy + permission slug
- validasi memakai FormRequest
...
- Remember me 7 hari
- Middleware active untuk blokir user nonaktif
- CRUD user di area admin
- Deactivate user terpisah dari delete
- Filter index user berdasarkan tenant, branch, role, status, dan keyword
- Super admin global lintas tenant
- Owner global di tenant sendiri
...
- jika branch dipilih, tenant akan diambil dari branch tersebut
- branch harus berada pada tenant yang sama
- tenant nonaktif tidak boleh dipilih
- non platform admin tidak bisa memilih tenant lain
- non global user tidak bisa memilih branch lain
- role global selain super_admin tidak boleh punya branch_id
...
User index:
- super_admin bisa filter tenant dan branch
...
Seeder yang relevan:
- TenantSeeder
- RoleSeeder (4 role: super_admin, owner, admin, kasir)
- PermissionSeeder
...
- T0102BranchScopingTest
- T0104RoleConstraintAndAuditImmutabilityTest
- T0105DeactivateUserTest
- T0106RememberMeTest

Coverage yang sudah terbukti:
- create user
- username unique per tenant
- role global vs branch-scoped constraints
- owner bisa dibuat tanpa branch_id
...
- data user sudah tenant-aware
- data user operasional sudah branch-aware
- role global tenant untuk owner sudah ada
- audit trail sudah ada
- super admin platform-level sudah ada

Belum ada di modul ini:
- provisioning tenant self-service
- manajemen permission granular via UI
- approval flow user
